...
export class RegisterPage implements OnInit {
  ...
  constructor(private toastCtrl: ToastController, private router: Router, private firebase: FirebaseService) {
    try {
      addIcons({ 'person': personIcon, 'mail': mailIcon, 'lock-closed': lockClosedIcon, 'lock-open': lockOpenIcon });
    } catch (e) { }
  }
  ...
  async onSubmit(ev: Event){
    ...
    try {
      if (!this.firebase) {
        await this.showToast('Error: Servicio de Firebase no disponible.', 'danger');
        return;
      }
      
      const exists = await this.firebase.isEmailRegistered(email);
      if (exists) {
        await this.showToast('El correo ya está registrado.', 'danger');
        return;
      }
      
      await this.firebase.saveUser({ name, email, password });
      await this.showToast('Cuenta creada correctamente', 'success');
    } catch (e: any) {
      console.error('Error al registrar usuario en Firebase:', e);
      const errorMsg = e?.message || 'Error desconocido';
      await this.showToast(`Error al crear la cuenta: ${errorMsg}`, 'danger');
      return;
    }
